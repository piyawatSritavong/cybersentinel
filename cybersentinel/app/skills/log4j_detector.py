...
def execute_log4j_detector(log_entries, http_headers):
    ...
    try:
        # Define the pattern to match JNDI lookup strings
        pattern = r'\${jndi:(ldap|ldaps|dns):\/\/[^}]+}'
        
        # Analyze log entries
        for entry in log_entries:
            if re.search(pattern, entry):
                result['result'] = True
                result['details'].append(f"Log entry: {entry} contains a potential Log4j exploit pattern")
        
        # Analyze HTTP headers
        for header, value in http_headers.items():
            if re.search(pattern, value):
                result['result'] = True
                result['details'].append(f"HTTP header '{header}': {value} contains a potential Log4j exploit pattern")
        
        if result['result']:
            logging.warning("Potential Log4j exploit pattern detected")
        else:
            logging.info("No potential Log4j exploit patterns detected")
